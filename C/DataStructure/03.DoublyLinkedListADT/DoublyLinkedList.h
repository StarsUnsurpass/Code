...
// 用户只能看到这些类型的指针，而无法访问其内部结构 ——OOP封装
...
typedef int (*CompareFunc)(const void* data1, const void* datab);
typedef void (*PrintFunc)(const void* data);
typedef void (*FreeFunc)(void* data);
typedef void (*ActionFunc)(void* data, void* context);

// MemoryPool 相关函数指针类型
// 内存池结构，预先分配一块内存，用于存储链表节点，减少频繁的内存分配和释放操作
// 将其划分为多个固定大小的块，以便快速分配和释放，内部通过一个单向链表来管理这些内存块
